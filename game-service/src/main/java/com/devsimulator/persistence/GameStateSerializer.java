...
public class GameStateSerializer {
    ...
    public GameEngineSnapshot fromJson(String json) {
        try {
            ...
            if (node.isObject() && !node.has("scheduledDailyStandup")) {
                ((com.fasterxml.jackson.databind.node.ObjectNode) node).putNull("scheduledDailyStandup");
            }
            if (node.isObject() && !node.has("sprintSyncAttendedToday")) {
                ((com.fasterxml.jackson.databind.node.ObjectNode) node).put("sprintSyncAttendedToday", false);
            }
