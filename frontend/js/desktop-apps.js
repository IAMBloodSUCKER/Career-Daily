...
const DESKTOP_APPS = [
    ...
    { id: 'jenkins', label: 'Jenkins', emoji: 'J', loadMs: 1500, loadText: 'Loading Jenkins…' },
    ...
    { id: 'vpn', label: 'Cisco', emoji: '🔒', loadMs: 1800, loadText: 'Cisco AnyConnect — проверка профиля…' },
...
/** Локально на ноутбуке — без корпоративного VPN */
const LOCAL_DESKTOP_APP_IDS = new Set([
    'vpn', 'recycle', 'ide', 'terminal', 'kitty', 'docker', 'word', 'excel'
]);

function requiresCorporateVpn(appId) {
    if (!appId) return false;
    if (LOCAL_DESKTOP_APP_IDS.has(appId)) return false;
    if (appId === 'contacts' || appId === 'coffee') return false;
    return !!getDesktopApp(appId);
}
...
function createAppIconButton(app, onOpen) {
    ...
    if (onOpen) btn.onclick = () => onOpen(app.id);
    return btn;
}
...
function createStartMenuTile(app, onOpen) {
    ...
    btn.addEventListener('contextmenu', e => {
        e.preventDefault();
        e.stopPropagation();
        if (typeof showDesktopContextMenu !== 'function' || typeof pinAppToDesktop !== 'function') return;
        const pinLabel = typeof t === 'function' ? t('desktop.pin') : 'Закрепить на рабочем столе';
...
function getWallpaperIndex() {
    ...
    return ((raw % WALLPAPER_COUNT) + WALLPAPER_COUNT) % WALLPAPER_COUNT;
}

function applyGameWallpaper(element) {
    if (!element) return;
    element.className = 'game-wallpaper wallpaper-' + getWallpaperIndex();
}
...
function addLimitedActions(container, actions) {
    ...
    actions.forEach(opt => {
        const btn = document.createElement('button');
        btn.className = 'limited-btn' + (opt.className ? ' ' + opt.className : '');
...
function wireBrandSidebarTree(root, options = {}) {
    const sidebar = root.querySelector(options.sidebarSelector || '.brand-sidebar');
    if (!sidebar) return;

    const sqlEl = options.sqlSelector ? root.querySelector(options.sqlSelector) : null;
    const outEl = options.outSelector ? root.querySelector(options.outSelector) : null;
    const nodes = options.nodes || {};

    const selectNode = (item) => {
        sidebar.querySelectorAll('.brand-tree-item[data-tree-node]').forEach(n => n.classList.remove('active'));
        item.classList.add('active');
        const key = item.dataset.treeNode;
        if (key && nodes[key] !== undefined && sqlEl) {
            sqlEl.value = typeof nodes[key] === 'string' ? nodes[key] : nodes[key].sql;
        }
        if (outEl && options.resetOutput !== undefined) outEl.textContent = options.resetOutput;
        if (options.onSelect) options.onSelect(key, item);
    };

    sidebar.querySelectorAll('[data-tree-node]').forEach(item => {
        item.setAttribute('role', 'button');
        item.tabIndex = 0;

        const activate = (e) => {
            if (item.dataset.treeToggle === 'true') {
                e?.preventDefault?.();
                const parentKey = item.dataset.treeNode;
                const children = sidebar.querySelector(`[data-tree-parent="${parentKey}"]`);
                const open = item.classList.toggle('expanded');
                item.classList.toggle('collapsed', !open);
                if (children) children.classList.toggle('collapsed', !open);
                const chevron = item.querySelector('.brand-tree-chevron');
...
function taskIncomplete(ticketId) {
    const t = getTaskByTicket(ticketId);
    return !!t && !t.completed;
}

/** Активный шаг SQL-задачи в pgAdmin (CHECK_METRICS). */
function getActiveSqlPgStep() {
    for (const task of workspace?.tasks || []) {
        if (task.completed || task.scenarioTag !== 'SQL_ANALYTICS') continue;
        for (const o of task.objectives || []) {
            if (o.completed) continue;
            if (o.type === 'CHECK_METRICS' && o.contactId === 'postgresql' && o.messageId) {
                return { task, objective: o, actionId: o.messageId };
            }
        }
    }
    return null;
...
const SQL_PG_ACTION_META = {
    'sql-salary-dept': {
        button: '📊 HR: зарплаты по отделам',
        sql: `-- hr schema · SQL-301\nSELECT department, ROUND(AVG(salary)) AS avg_salary, COUNT(*) AS cnt\nFROM hr.employees\nGROUP BY department\nHAVING AVG(salary) > 65000\nORDER BY avg_salary DESC;`,
        treeNode: 'employees',
        hint: 'Средняя зарплата по отделам (AVG > 65000). Нажмите кнопку внизу — результат попадёт в чеклист.'
    },
    'sql-budget-overrun': {
        button: '📊 Бюджет проектов',
        sql: `-- hr schema · SQL-302\nSELECT d.name, d.budget AS dept_budget, SUM(p.budget) AS projects_sum\nFROM hr.departments d\nJOIN hr.projects p ON p.department_id = d.id\nGROUP BY d.name, d.budget\nHAVING SUM(p.budget) > d.budget;`,
        treeNode: 'departments',
        hint: 'Отделы, где сумма бюджетов проектов превышает бюджет отдела.'
    },
    'sql-max-age': {
        button: '📊 Person: MAX(age)',
        sql: `-- hr schema · SQL-303\nSELECT name, MAX(age) AS max_age\nFROM hr.person\nGROUP BY name\nORDER BY name;`,
        treeNode: 'person',
        hint: 'Максимальный возраст для каждого имени в таблице person.'
    },
    'sql-person-dupes': {
        button: '📊 Person: дубликаты name+age',
        sql: `-- hr schema · SQL-304\nSELECT name, age, COUNT(*) AS cnt\nFROM hr.person\nGROUP BY name, age\nHAVING COUNT(*) > 1;`,
        treeNode: 'person',
        hint: 'Полные дубликаты пар name + age (HAVING COUNT(*) > 1).'
    }
};

function getSqlWorkflowGuide(task) {
    if (!task || task.scenarioTag !== 'SQL_ANALYTICS') return null;
    const pgStep = (task.objectives || []).find(o => o.type === 'CHECK_METRICS' && o.contactId === 'postgresql');
    const meta = pgStep?.messageId ? SQL_PG_ACTION_META[pgStep.messageId] : null;
    const replyOpt = (task.replyOptions || []).find(o => o.correct);
    return {
        ticketId: task.ticketId,
        pgActionId: pgStep?.messageId,
        pgButton: meta?.button?.replace(/^📊\s*/, '') || pgStep?.label,
        slackContact: 'dmitry',
        slackReply: replyOpt?.text,
        hint: meta?.hint || task.description
    };
}

/** Есть ли незавершённый ops-шаг (CHECK_METRICS / HEAP_DUMP) с appId+actionId. */
function taskNeedsOps(appId, actionId) {
    return (workspace?.tasks || []).some(t => {
        if (t.completed) return false;
        return (t.objectives || []).some(o =>
            !o.completed && (o.type === 'CHECK_METRICS' || o.type === 'HEAP_DUMP')
            && o.contactId === appId && o.messageId === actionId);
    });
}

function taskActiveTroubleshoot() {
    return (workspace?.tasks || []).some(t => !t.completed && t.scenarioTag === 'TROUBLESHOOT_DIAG');
}

/** Завершить CHECK_METRICS / HEAP_DUMP objective, если appId+actionId совпадают с активной задачей. */
function tryOpsObjective(appId, actionId) {
    for (const task of workspace?.tasks || []) {
        if (task.completed) continue;
        for (const o of task.objectives || []) {
            if (o.completed) continue;
            if ((o.type === 'CHECK_METRICS' || o.type === 'HEAP_DUMP')
                && o.contactId === appId && o.messageId === actionId) {
                opsAction(appId, actionId);
                return true;
            }
        }
    }
...
function getKubectlGetPodsOutput() {
    const hasTshOom = taskNeedsOps('kubernetes', 'oom-pods') || taskNeedsOps('grafana', 'oom-heap-growth');
    const hasOom = taskActiveByTag('MEMORY_LEAK') || hasTshOom;
...
function wireK8sApp(root, ctx) {
    const { hasOom, hasTshOom, hasTshImport, hasRelCanary, hasInc501, hasKafka } = ctx;
    const needsOomPods = () => taskNeedsOps('kubernetes', 'oom-pods');
    const needsImportPods = () => taskNeedsOps('kubernetes', 'import-pods');
    const needsHeapDump = () => taskNeedsOps('kubernetes', 'heap-dump');
    ...
    root.querySelectorAll('[data-k8s-action]').forEach(btn => {
        btn.addEventListener('click', (e) => {
            ...
            if (action === 'get-pods') {
                ...
                if (needsOomPods()) tryOpsObjective('kubernetes', 'oom-pods');
                else if (needsImportPods()) tryOpsObjective('kubernetes', 'import-pods');
            ...
            } else if (action === 'heap-dump' && (hasOom || needsHeapDump())) {
                ...
                tryOpsObjective('kubernetes', 'heap-dump');
                status('Heap dump captured');
            } else if (action === 'canary' && hasRelCanary) {
                term('kubectl argo rollouts promote checkout-api-canary -n checkout',
                    'rollout checkout-api-canary promoted\nStable: 100% · Canary: 0%\nError rate 0.02% — within SLO');
                tryOpsObjective('kubernetes', 'rollout-canary');
                status('Canary promoted to 100%');
            } else if (action === 'oom-events' && (hasOom || hasTshOom)) {
...
function getK8sAlertBanner() {
    ...
    const tsh = (workspace?.tasks || []).find(t => !t.completed && t.scenarioTag === 'TROUBLESHOOT_DIAG');
    if (tsh) return `SEV-2: ${tsh.ticketId} — ${tsh.title}`;
    if (taskIncomplete('REL-502')) return 'Canary 10% — мониторьте error rate перед promote';
...
function getK8sTopPodsText() {
    const hasOom = taskActiveByTag('MEMORY_LEAK') || taskNeedsOps('grafana', 'oom-heap-growth');
    const hasInc501 = taskActiveByTag('RACE_CONDITION');
    return [
...
function wireGrafanaApp(root, ctx) {
    ...
    root.querySelectorAll('[data-gf-panel]').forEach(panel => {
        panel.addEventListener('click', () => {
            ...
            if (id === 'heap' && (ctx.hasOom || ctx.hasTshOom)) {
                setStatus('Analyzing JVM heap chart…');
                if (taskNeedsOps('grafana', 'oom-heap-growth')) tryOpsObjective('grafana', 'oom-heap-growth');
                else opsAction('grafana', 'heap-chart');
    ...
    root.querySelectorAll('[data-gf-action]').forEach(btn => {
        btn.addEventListener('click', (e) => {
            ...
            } else if (action === 'db-conn') {
                setStatus('PostgreSQL: active sessions 12 / max 20 · NSI pool 7/15');
                tryOpsObjective('grafana', 'db-connections');
            } else if (action === 'p99') {
                setStatus('p99 latency — checkout-api SLO dashboard');
                tryOpsObjective('grafana', 'p99-dashboard');
            } else if (action === 'canary-err') {
                setStatus('Canary error rate 0.02% — within SLO');
                tryOpsObjective('grafana', 'canary-errors');
            } else if (action === 'explorer-observe') {
                setStatus('График просмотрен — вернитесь в Slack');
                opsAction('grafana', 'step-grafana');
            } else if (action === 'integration') {
                setStatus('Task: integration metrics');
                if (!tryOpsObjective('grafana', 'integration-metrics')
                    && !tryOpsObjective('grafana', 'integration-dashboard')) {
                    opsAction('grafana', ctx.hasObs ? 'integration-dashboard' : 'integration-metrics');
                }
...
function renderPortalApp(container) {
        ...
        return `
            <div class="portal-pay-header">
                <div class="portal-pay-stat portal-pay-stat-main">
                    ...
                    <span class="portal-pay-value">${formatMoney(monthly)}</span>
                    <span class="portal-pay-hint">${typeof formatPayGrade === 'function' ? formatPayGrade(workspace) : (workspace.payGrade || '')}</span>
                </div>
                <div class="portal-pay-stat">
                    <span class="portal-pay-label">${portalLabel('portal.pay.daily', 'За продуктивный день')}</span>
                    <span class="portal-pay-value">${formatMoney(daily)}</span>
                ...
                <div class="portal-pay-stat">
                    ...
                    <span class="portal-pay-value">${formatMoney(balance)}</span>
            ...
            <div class="portal-cards">
                <article class="portal-card">
                    ...
                    <p>${portalLabel('portal.pay.nextReview', 'Следующий пересмотр')}: <strong>${typeof formatSalaryReviewLabel === 'function' ? formatSalaryReviewLabel(workspace.nextSalaryReview, p.day) : (workspace.nextSalaryReview || '—')}</strong></p>
                ...
                <article class="portal-card">
                    ...
                    <ul class="portal-list portal-benefits">
                        ...
                        <li>📚 ${portalLabel('portal.benefit.learning', 'Бюджет на обучение 50 000 ₽/год')}</li>
                ...
                <article class="portal-card portal-card-wide">
                    ...
                    <p>${portalLabel('portal.pay.howText', 'Оклад gross зависит от грейда. В конце дня — выплата ≈ оклад ÷ 22, пропорционально закрытым задачам.')}</p>
                </article>
            </div>`;
    }
    ...
    function paint() {
        ...
        content.querySelectorAll('[data-slack]').forEach(btn => {
            btn.onclick = () => {
                selectSlackContact(btn.dataset.slack);
                openAppWindow('slack');
            };
        });
    }
...
function outlookLabel(key, fallback) {
    if (typeof t !== 'function') return fallback;
    const text = t(key);
...
const CALENDAR_STATUS_META = {
    upcoming: { label: 'Скоро', cls: 'outlook-cal-status--upcoming' },
    live: { label: 'Сейчас', cls: 'outlook-cal-status--live' },
    completed: { label: 'Завершено', cls: 'outlook-cal-status--done' },
    missed: { label: 'Пропущено', cls: 'outlook-cal-status--missed' }
};

function renderEmailApp(container) {
    const tab = container.dataset.outlookTab || 'mail';
    const day = workspace?.player?.day || 1;
    const time = workspace?.timeLabel || '09:00';

    container.innerHTML = appShell(
        '📧 Outlook',
        outlookLabel('outlook.subtitle', 'Почта и календарь · синхронизация с Meet / JIRA'),
        `<div class="outlook-shell">
            <nav class="outlook-tabs">
                <button type="button" class="outlook-tab${tab === 'mail' ? ' active' : ''}" data-outlook-tab="mail">
                    ${outlookLabel('outlook.tab.mail', 'Почта')}
                </button>
                <button type="button" class="outlook-tab${tab === 'calendar' ? ' active' : ''}" data-outlook-tab="calendar">
                    ${outlookLabel('outlook.tab.calendar', 'Календарь')}
                </button>
            </nav>
            <div id="outlook-panel" class="outlook-panel"></div>
        </div>`
    );

    container.querySelectorAll('[data-outlook-tab]').forEach(btn => {
        btn.onclick = () => {
            container.dataset.outlookTab = btn.dataset.outlookTab;
            renderEmailApp(container);
        };
    });

    const panel = container.querySelector('#outlook-panel');
    if (tab === 'calendar') {
        renderOutlookCalendar(panel);
    } else {
        renderOutlookMail(panel, container);
    }
}

function renderOutlookMail(panel, container) {
    const emails = workspace.emails || [];
    let selectedId = container.dataset.selectedEmail || emails.find(e => !e.read)?.id || emails[0]?.id;
    const selected = emails.find(e => e.id === selectedId);

    panel.innerHTML = `<div class="outlook-layout">
        <div class="outlook-list" id="outlook-list"></div>
        <div class="outlook-reading" id="outlook-reading"></div>
    </div>`;

    const listEl = panel.querySelector('#outlook-list');
    emails.forEach(em => {
    ...
    const readEl = panel.querySelector('#outlook-reading');
    if (!selected) {
...
function renderOutlookCalendar(panel) {
    const events = workspace.calendarEvents || [];
    const day = workspace?.player?.day || 1;
    const time = workspace?.timeLabel || '09:00';
    const company = workspace?.projectCompany || 'Workspace';

    panel.innerHTML = `<div class="outlook-calendar">
        <header class="outlook-cal-head">
            <div>
                <h3 class="outlook-cal-title">${outlookLabel('outlook.cal.today', 'Сегодня')}</h3>
                <p class="outlook-cal-meta">${company} · ${outlookLabel('outlook.cal.day', 'День')} ${day} · ${outlookLabel('outlook.cal.now', 'Сейчас')} ${time}</p>
            </div>
            <span class="outlook-cal-range">09:00 – 17:00</span>
        </header>
        <div class="outlook-cal-timeline" id="outlook-cal-timeline"></div>
    </div>`;

    const list = panel.querySelector('#outlook-cal-timeline');
    if (!events.length) {
        list.innerHTML = `<p class="hint-text">${outlookLabel('outlook.cal.empty', 'Нет событий на сегодня')}</p>`;
        return;
    }

    events.forEach(ev => {
        const statusKey = ev.status || 'upcoming';
        const statusMeta = CALENDAR_STATUS_META[statusKey] || CALENDAR_STATUS_META.upcoming;
        const statusLabel = outlookLabel('outlook.cal.status.' + statusKey, statusMeta.label);
        const row = document.createElement('article');
        row.className = 'outlook-cal-event outlook-cal-event--' + statusKey;
        row.innerHTML = `
            <div class="outlook-cal-time">
                <span class="outlook-cal-start">${ev.startTime}</span>
                <span class="outlook-cal-end">${ev.endTime}</span>
            </div>
            <div class="outlook-cal-body">
                <div class="outlook-cal-row">
                    <strong class="outlook-cal-event-title">${ev.title}</strong>
                    <span class="outlook-cal-status ${statusMeta.cls}">${statusLabel}</span>
                </div>
                ${ev.subtitle ? `<p class="outlook-cal-sub">${ev.subtitle}</p>` : ''}
                <p class="outlook-cal-loc">
                    ${ev.location ? `📍 ${ev.location}` : ''}
                    ${ev.organizer ? ` · ${ev.organizer}` : ''}
                </p>
            </div>`;

        if (ev.joinable && ev.meetingId && workspace.pendingMeeting?.id === ev.meetingId) {
            const join = document.createElement('button');
            join.type = 'button';
            join.className = 'outlook-meeting-join outlook-cal-join';
            join.textContent = outlookLabel('meeting.joinOnline', 'Присоединиться');
            join.onclick = () => {
                closeApp('email');
                startMeeting(workspace.pendingMeeting);
            };
            row.querySelector('.outlook-cal-body').appendChild(join);
        }
...
function renderKafkaApp(container) {
    const hasInc501 = taskActiveByTag('RACE_CONDITION');
    const hasKafka = taskActiveByTag('KAFKA_CONSUMER') || taskNeedsOps('kafka', 'consumer-lag');
    ...
    addLimitedActions(container, [
        { text: '📊 Check consumer lag (задача)', disabled: !hasKafka, action: () => tryOpsObjective('kafka', 'consumer-lag') || opsAction('kafka', 'consumer-lag') },
...
function renderGrafanaApp(container) {
    const hasInc501 = taskActiveByTag('RACE_CONDITION');
    const hasTshOom = taskNeedsOps('grafana', 'oom-heap-growth');
    const hasTshImport = taskNeedsOps('grafana', 'db-connections');
    const hasTshP99 = taskNeedsOps('grafana', 'p99-dashboard');
    const hasTshIntegration = taskNeedsOps('grafana', 'integration-metrics');
    const hasRelCanary = taskIncomplete('REL-502');
    const hasOom = taskActiveByTag('MEMORY_LEAK') || hasTshOom;
    const hasKafka = taskActiveByTag('KAFKA_CONSUMER') || taskNeedsOps('kafka', 'consumer-lag');
    const hasObs = taskActiveAnyTag('METRICS_SLA', 'SQL_SLOW_QUERY') || hasTshP99 || hasTshIntegration;
    const hasAlert = hasInc501 || hasOom || hasKafka || hasObs || hasTshImport || hasRelCanary || taskActiveTroubleshoot();
    ...
    const firingAlerts = (hasInc501 ? 2 : 0) + (hasOom ? 1 : 0) + (hasKafka ? 1 : 0) + (hasObs ? 1 : 0)
        + (hasTshImport ? 1 : 0) + (hasRelCanary ? 1 : 0);
    const explorerGrafanaStep = typeof isNoCodeMode === 'function' && isNoCodeMode()
        && workspace?.tasks?.some(t => typeof findGuidedObjective === 'function' && findGuidedObjective(t, 'step-grafana'));
        ...
        `<div class="gf-layout">
            ...
            <section class="gf-main">
                ...
                <div class="gf-action-bar">
                    ${explorerGrafanaStep ? `<button type="button" class="gf-action-btn gf-action-btn--explorer" data-gf-action="explorer-observe">👁 Посмотрел график</button>` : ''}
                    <button type="button" class="gf-action-btn" data-gf-action="heap" ${hasOom ? '' : 'disabled'}>📈 Analyze heap</button>
                    <button type="button" class="gf-action-btn" data-gf-action="db-conn" ${hasTshImport ? '' : 'disabled'}>🐘 DB connections</button>
                    <button type="button" class="gf-action-btn" data-gf-action="p99" ${hasTshP99 || hasObs ? '' : 'disabled'}>⏱ p99 latency</button>
                    <button type="button" class="gf-action-btn" data-gf-action="canary-err" ${hasRelCanary ? '' : 'disabled'}>📊 Canary errors</button>
                    <button type="button" class="gf-action-btn" data-gf-action="integration" ${hasKafka || hasObs || hasTshIntegration ? '' : 'disabled'}>🌐 Integration metrics</button>
    ...
    wireGrafanaApp(root, { hasOom, hasTshOom, hasTshImport, hasTshP99, hasTshIntegration, hasRelCanary, hasKafka, hasObs, hasAlert, hasInc501, firingAlerts });
}

function renderK8sApp(container) {
    const hasTshOom = taskNeedsOps('grafana', 'oom-heap-growth') || taskNeedsOps('kubernetes', 'oom-pods');
    const hasTshImport = taskNeedsOps('kubernetes', 'import-pods');
    const hasHeapDump = taskNeedsOps('kubernetes', 'heap-dump');
    const hasRelCanary = taskIncomplete('REL-502');
    const hasOom = taskActiveByTag('MEMORY_LEAK') || hasTshOom;
    const hasInc501 = taskActiveByTag('RACE_CONDITION');
    const hasKafka = taskActiveByTag('KAFKA_CONSUMER');
        ...
        `<div class="k8s-layout">
            ...
            <section class="k8s-main">
                ...
                <div class="k8s-action-bar">
                    ...
                    <button type="button" class="k8s-action-btn" data-k8s-action="canary" ${hasRelCanary ? '' : 'disabled'}>canary promote</button>
                    <button type="button" class="k8s-action-btn" data-k8s-action="heap-dump" ${hasOom || hasHeapDump ? '' : 'disabled'}>heap dump</button>
                ...
                ${hasTshImport ? '<p class="k8s-hint">Import batch: gateway 3/3 Running, backend 2/2 Running — проверьте conn pool.</p>' : ''}
                <div class="k8s-status" id="k8s-status">namespace checkout · context prod-cluster</div>
            </section>
        </div>`);

    wireK8sApp(root, { hasOom, hasTshOom, hasTshImport, hasRelCanary, hasInc501, hasKafka });
}
...
function renderPostmanApp(container) {
    const relStaging = taskIncomplete('REL-501');
    ...
    addLimitedActions(container, [
        ...
        { text: '✅ Smoke staging', disabled: !relStaging, action: () => {
            document.getElementById('postman-resp').textContent = '200 OK\nGET /actuator/health → UP\nPOST /api/checkout → 201\nAll smoke tests passed on staging';
            tryOpsObjective('postman', 'smoke-staging');
        }},
    ...
    const root = mountBrandedApp(container, 'brand-excel', `
        ...
        `<div class="excel-sheet">
            <table class="excel-table" id="excel-table">
                ...
                ${rows || (() => {
                    const prOwner = workspace?.contacts?.find(c => c.id === 'alex')?.name || '—';
                    return `<tr><td>JIRA-142</td><td>In Progress</td><td>3</td><td>${workspace.player.name}</td></tr>
                <tr><td>PR-247</td><td>Review</td><td>2</td><td>${prOwner}</td></tr>
...
function renderOpenSearchApp(container) {
    const hasKafka = taskActiveByTag('KAFKA_CONSUMER') || taskNeedsOps('kafka', 'consumer-lag');
    const hasOsTsh = taskNeedsOps('opensearch', 'integration-errors');
    const hasObs = taskActiveAnyTag('METRICS_SLA', 'SQL_SLOW_QUERY') || hasOsTsh;
    ...
    addLimitedActions(container, [
        { text: '🔍 Integration 504 errors', disabled: !(hasKafka || hasOsTsh), action: () => tryOpsObjective('opensearch', 'integration-errors') || opsAction('opensearch', 'integration-errors') },
...
function renderPostgresApp(container) {
    const sqlStep = getActiveSqlPgStep();
    const sqlMeta = sqlStep ? SQL_PG_ACTION_META[sqlStep.actionId] : null;
    const defaultSql = sqlMeta?.sql || 'SELECT id, customer_id, total FROM orders WHERE id = 142;';
    ...
    const root = mountBrandedApp(container, 'brand-pgadmin', `
        ...
        `${sqlStep ? `<div class="brand-task-banner pg-task-banner">
            <strong>${escapeHtml(sqlStep.task.ticketId)}</strong>
            <span>${escapeHtml(sqlMeta?.hint || sqlStep.task.title)}</span>
            <span class="brand-task-banner-step">Шаг 2/4: pgAdmin → кнопка «${escapeHtml((sqlMeta?.button || '').replace(/^📊\\s*/, ''))}» внизу окна</span>
        </div>` : ''}
        <div class="brand-split">
            <aside class="brand-sidebar pg-sidebar">
                <div class="brand-tree-item" data-tree-node="server">🐘 checkout (prod)</div>
                <div class="brand-tree-item indent brand-tree-folder expanded" data-tree-node="schemas" data-tree-toggle="true">
                    <span class="brand-tree-chevron">▾</span> Schemas · public
                </div>
                <div class="brand-tree-children" data-tree-parent="schemas">
                    <div class="brand-tree-item indent2" data-tree-node="orders">public.orders</div>
                    <div class="brand-tree-item indent2" data-tree-node="customers">public.customers</div>
                </div>
                <div class="brand-tree-item indent brand-tree-folder expanded" data-tree-node="hr" data-tree-toggle="true">
                    <span class="brand-tree-chevron">▾</span> Schemas · hr
                </div>
                <div class="brand-tree-children" data-tree-parent="hr">
                    <div class="brand-tree-item indent2${sqlMeta?.treeNode === 'employees' ? ' active' : ''}" data-tree-node="employees">hr.employees</div>
                    <div class="brand-tree-item indent2${sqlMeta?.treeNode === 'departments' ? ' active' : ''}" data-tree-node="departments">hr.departments</div>
                    <div class="brand-tree-item indent2${sqlMeta?.treeNode === 'person' ? ' active' : ''}" data-tree-node="person">hr.person</div>
                    <div class="brand-tree-item indent2" data-tree-node="projects">hr.projects</div>
                </div>
            </aside>
            <section class="brand-panel">
                <div class="brand-tab active">Query Tool</div>
                <textarea class="sql-editor brand-sql pg-sql" rows="5">${escapeHtml(defaultSql)}</textarea>
                <pre class="tool-log brand-output pg-out">${sqlStep ? 'Нажмите кнопку задачи внизу ↓' : 'Execute query →'}</pre>
            </section>
        </div>`);
    const out = root.querySelector('.pg-out');
    root.querySelector('.pg-dbeaver-btn')?.addEventListener('click', () => openAppWindow('dbeaver'));
    wireBrandSidebarTree(root, {
        sidebarSelector: '.pg-sidebar',
        sqlSelector: '.pg-sql',
        outSelector: '.pg-out',
        resetOutput: sqlStep ? 'Нажмите кнопку задачи внизу ↓' : 'Execute query →',
        nodes: {
            server: 'SELECT version();\n-- Connected: checkout@db.prod:5432',
            orders: 'SELECT id, customer_id, total FROM orders WHERE id = 142;',
            customers: 'SELECT id, email, name, created_at\nFROM customers\nORDER BY id LIMIT 50;',
            employees: SQL_PG_ACTION_META['sql-salary-dept'].sql,
            departments: SQL_PG_ACTION_META['sql-budget-overrun'].sql,
            person: SQL_PG_ACTION_META['sql-max-age'].sql,
            projects: 'SELECT id, name, budget, department_id FROM hr.projects ORDER BY budget DESC;'
        }
    });
    ...
    const pgButtons = [
        { text: '▶ Execute (read-only)', action: () => {
            ...
            runPgOpsIfActive('pg-slow-queries');
        }},
        { text: 'EXPLAIN ANALYZE', action: () => {
            out.textContent = 'Seq Scan on orders — cost 842ms (slow!)';
            runPgOpsIfActive('explain-orders');
        }},
        { text: '📊 HR: зарплаты по отделам', actionId: 'sql-salary-dept', action: () => {
            out.textContent = 'department | avg_salary | cnt\nIT | 78333 | 3\nSales | 67500 | 2\nHR | 58000 | 2';
            runPgOpsIfActive('sql-salary-dept');
            if (typeof showToast === 'function') showToast('✅ SQL выполнен — отправьте результат Dmitry в Slack');
        }},
        { text: '📊 Бюджет проектов', actionId: 'sql-budget-overrun', action: () => {
            out.textContent = 'IT dept 500000 vs projects sum 530000 — overrun 30000';
            runPgOpsIfActive('sql-budget-overrun');
            if (typeof showToast === 'function') showToast('✅ SQL выполнен — отправьте результат Dmitry в Slack');
        }},
        { text: '📊 Person: MAX(age)', actionId: 'sql-max-age', action: () => {
            out.textContent = 'name | max_age\nИван | 35\nПетр | 40\nМария | 32';
            tryOpsObjective('postgresql', 'sql-max-age');
            if (typeof showToast === 'function') showToast('✅ SQL выполнен — отправьте результат Dmitry в Slack');
        }},
        { text: '📊 Person: дубликаты name+age', actionId: 'sql-person-dupes', action: () => {
            out.textContent = 'name | age | cnt\n(нет полных дубликатов — HAVING COUNT(*) > 1)';
            tryOpsObjective('postgresql', 'sql-person-dupes');
            if (typeof showToast === 'function') showToast('✅ SQL выполнен — отправьте результат Dmitry в Slack');
        }},
        { text: 'INSERT/UPDATE', disabled: true }
    ];

    const activeActionId = sqlStep?.actionId;
    const filtered = activeActionId
        ? pgButtons.filter(b => b.disabled || b.actionId === activeActionId || !b.actionId)
        : pgButtons;

    addLimitedActions(root, filtered.map(b => {
        const btn = { text: b.text, disabled: b.disabled, action: b.action };
        if (b.actionId === activeActionId) btn.className = 'limited-btn-task-active';
        return btn;
    }));
}

function runPgOpsIfActive(actionId) {
    tryOpsObjective('postgresql', actionId);
}
...
function jenkinsStatusBall(status) {
    const map = { SUCCESS: 'blue', UNSTABLE: 'yellow', FAILURE: 'red', ABORTED: 'grey' };
    const tone = map[status] || 'grey';
    return `<span class="jenkins-ball jenkins-ball--${tone}" title="${status}" aria-label="${status}"></span>`;
}

function jenkinsWeatherIcon(health) {
    const labels = { sunny: 'Sunny', cloudy: 'Cloudy', stormy: 'Stormy' };
    return `<span class="jenkins-weather jenkins-weather--${health}" title="${labels[health] || health}"></span>`;
}
...
function renderJenkinsApp(container) {
    ...
    const weather = failed ? 'stormy' : 'sunny';
    const latestStatus = failed ? 'FAILURE' : 'SUCCESS';
    const builds = [
        { num: 248, branch: 'main', status: latestStatus, time: '4m 12s', ago: '2 min ago' },
        { num: 247, branch: 'PR-247', status: 'UNSTABLE', time: '3m 45s', ago: '1 hr ago' },
        { num: 246, branch: 'main', status: 'SUCCESS', time: '4m 01s', ago: '3 hr ago' }
    ];
    const historyHtml = builds.map(b => `
        <li class="jenkins-history-item${b.num === 248 ? ' active' : ''}">
            ${jenkinsStatusBall(b.status)}
            <a href="#" class="jenkins-build-link" onclick="return false">#${b.num}</a>
            <span class="jenkins-build-meta">${b.ago}</span>
        </li>`).join('');
    const consoleText = failed
        ? `Started by timer\nRunning on agent linux-worker-03 in /var/jenkins_home/workspace/checkout-backend\n[ERROR] ${failMsg}`
        : 'Finished: SUCCESS\nAll tests passed · artifact checkout-backend-2.4.1.jar';

    const root = mountBrandedApp(container, 'brand-jenkins', `
        <span class="jenkins-logo-mark" aria-hidden="true">J</span>
        <span class="jenkins-logo-text">Jenkins</span>
        <nav class="jenkins-breadcrumb" aria-label="Breadcrumb">
            <a href="#" onclick="return false">Dashboard</a>
            <span class="jenkins-bc-sep">»</span>
            <a href="#" onclick="return false">checkout-backend</a>
            <span class="jenkins-bc-sep">»</span>
            <span class="jenkins-bc-current">main</span>
        </nav>
        <span class="brand-spacer"></span>
        <input type="search" class="jenkins-search" placeholder="Search" aria-label="Search" disabled>
        <span class="brand-chip jenkins-user">dev</span>`,
        `<div class="jenkins-layout">
            <aside class="jenkins-sidebar" aria-label="Project navigation">
                <a class="jenkins-side-link active" href="#" onclick="return false">Status</a>
                <a class="jenkins-side-link" href="#" onclick="return false">Changes</a>
                <a class="jenkins-side-link" href="#" onclick="return false">Workspace</a>
                <a class="jenkins-side-link" href="#" onclick="return false">Build History</a>
                <a class="jenkins-side-link" href="#" onclick="return false">Modules</a>
                <a class="jenkins-side-link" href="#" onclick="return false">Trend</a>
            </aside>
            <section class="jenkins-main">
                <div class="jenkins-job-head">
                    ${jenkinsWeatherIcon(weather)}
                    <h2 class="jenkins-job-title">checkout-backend<span class="jenkins-branch"> / main</span></h2>
                </div>
                <div class="jenkins-job-toolbar">
                    <button type="button" class="jenkins-build-btn" id="jenkins-build-now" ${failed ? '' : 'disabled'} title="Build Now">
                        <span class="jenkins-play" aria-hidden="true"></span>
                        Build Now
                    </button>
                    <button type="button" class="jenkins-link-btn" id="jenkins-console-btn">Console Output</button>
                    <span class="jenkins-toolbar-spacer"></span>
                    <button type="button" class="jenkins-link-btn" id="jenkins-deploy-staging" ${relStaging ? '' : 'disabled'}>Deploy to staging</button>
                </div>
                <div class="jenkins-panels">
                    <div class="jenkins-history-pane">
                        <h3 class="jenkins-pane-title">Build History</h3>
                        <ul class="jenkins-history-list">${historyHtml}</ul>
                        <p class="jenkins-trend">${failed
                            ? 'Build stability: 67% (2 of 3 recent builds failed or unstable)'
                            : 'Build stability: 100%'}</p>
                    </div>
                    <div class="jenkins-detail-pane">
                        <div class="jenkins-permalinks">
                            <a href="#" onclick="return false">Last build (#248), ${failed ? '3 min' : '12 min'} ago</a>
                            <a href="#" onclick="return false">Last stable build (#246), 3 hr ago</a>
                            <a href="#" onclick="return false">Last successful build (#246), 3 hr ago</a>
                        </div>
                        <table class="jenkins-build-table">
                            <thead><tr><th></th><th>Build</th><th>Time Since</th><th>Duration</th></tr></thead>
                            <tbody>
                                ${builds.map(b => `<tr class="${b.num === 248 ? 'jenkins-row--selected' : ''}">
                                    <td>${jenkinsStatusBall(b.status)}</td>
                                    <td><a href="#" onclick="return false">#${b.num}</a> <span class="jenkins-build-branch">${b.branch}</span></td>
                                    <td>${b.ago}</td>
                                    <td>${b.time}</td>
                                </tr>`).join('')}
                            </tbody>
                        </table>
                        <div class="jenkins-console-wrap">
                            <div class="jenkins-console-head">Console Output — #248</div>
                            <pre id="jenkins-log" class="jenkins-console">${consoleText}</pre>
                        </div>
                    </div>
                </div>
            </section>
    ...
    const logEl = root.querySelector('#jenkins-log');
    const buildBtn = root.querySelector('#jenkins-build-now');
    const consoleBtn = root.querySelector('#jenkins-console-btn');
    const deployBtn = root.querySelector('#jenkins-deploy-staging');

    if (deployBtn && relStaging) {
        deployBtn.onclick = async () => {
            if (logEl) {
                logEl.textContent = 'Started by user dev\nDeploying checkout-backend-2.4.2.jar → staging...\n'
                    + 'Smoke hook pending · artifact uploaded\nFinished: SUCCESS';
            }
            tryOpsObjective('jenkins', 'deploy-staging');
            pushNotification('Jenkins', 'Deploy', 'Staging deploy SUCCESS — run Postman smoke', 'slack');
        };
    }

    if (buildBtn && failed) {
        buildBtn.onclick = async () => {
            logEl.textContent = 'Started by user dev\nBuilding on agent linux-worker-03\n...\n' + failMsg;
...
function renderMeetApp(container) {
    ...
    const body = container.querySelector('.app-body');
    if (workspace.meetingMissedToday && isStandupTimeReached()) {
        body.innerHTML = `<p class="hint-text">❌ Вы пропустили Daily Standup.</p>
    ...
    if (workspace.pendingMeeting) {
        ...
        body.innerHTML = '<p class="hint-text">Нет запланированных встреч. Следующий daily — завтра 11:30.</p>';
    }
}

function renderPrometheusApp(container) {
    const hasInc = taskActiveByTag('RACE_CONDITION');
    const hasObs = taskActiveAnyTag('METRICS_SLA', 'SQL_SLOW_QUERY') || taskNeedsOps('prometheus', 'partner-up');
    const hasP99 = taskNeedsOps('prometheus', 'p99-query');
    ...
    addLimitedActions(container, [
        { text: '▶ up{job="partner-api"}', disabled: !(hasObs || taskNeedsOps('prometheus', 'partner-up')), action: async () => {
            document.getElementById('prom-log').textContent = '→ 0 (1 target down)\npartner-api:9100 — connection refused';
            tryOpsObjective('prometheus', 'partner-up') || await opsAction('prometheus', 'partner-up');
        }},
        { text: '▶ histogram_quantile p99', disabled: !hasP99, action: async () => {
            document.getElementById('prom-log').textContent = '→ histogram_quantile(0.99, rate(http_request_duration_seconds_bucket[5m])) = 2.1s';
            tryOpsObjective('prometheus', 'p99-query') || await opsAction('prometheus', 'p99-query');
        }},
...
function renderVpnApp(container) {
    const connected = typeof isVpnConnected === 'function' && isVpnConnected();
    const company = (workspace?.projectCompany || 'corp').toLowerCase().replace(/\s+/g, '-');
    const log = document.getElementById('vpn-log');
    container.innerHTML = appShell('🔒 Cisco AnyConnect', `Корпоративный VPN · ${company}`, `
        <div class="vpn-cisco-brand">Cisco AnyConnect Secure Mobility Client</div>
        <div class="tool-row ${connected ? 'ok' : 'warn'}">
            ${connected ? '🟢 Подключено' : '🔴 Не подключено'}
            ${connected ? ' · vpn-gw.corp.internal' : ' · внутренние сервисы недоступны'}
        </div>
        <div class="tool-row">Профиль: <strong>${company}-vpn</strong></div>
        <div class="tool-row">Маршруты: Git, Grafana, почта (10.0.0.0/8)</div>
        <pre id="vpn-log" class="tool-log">${
            connected
                ? 'State: Connected · DPD: 45s · Assigned IP: 10.42.0.18'
                : 'State: Disconnected · подключитесь для доступа к корпоративным системам'
        }</pre>`);

    if (!connected) {
        addLimitedActions(container, [
            { text: '▶ Подключиться', action: () => {
                const el = document.getElementById('vpn-log');
                if (el) el.textContent = 'Contacting vpn-gw.corp.internal… Authenticating (MFA)…';
                setTimeout(() => {
                    if (typeof connectCorporateVpn === 'function') connectCorporateVpn();
                }, 1400);
            }}
        ]);
        return;
    ...
    addLimitedActions(container, [
        { text: '🔌 Отключить', action: () => {
            if (typeof disconnectCorporateVpn === 'function') disconnectCorporateVpn();
            const el = document.getElementById('vpn-log');
            if (el) el.textContent = 'Disconnected. Internal resources unavailable.';
            pushNotification('🔒 Cisco AnyConnect', 'VPN отключён', 'Корпоративные сервисы снова недоступны', 'warning');
        }}
    ]);
}
