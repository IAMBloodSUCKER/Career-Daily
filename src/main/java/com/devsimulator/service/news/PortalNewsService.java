...
public class PortalNewsService {
    ...
    public PortalNewsDto getNews(String lang, InteractiveGameEngine engine) {
        ...
        List<NewsItemDto> internal = buildInternalNews(profile, player.getDay(), player.getName());
    ...
    private List<NewsItemDto> buildInternalNews(ProjectProfile profile, int day, String playerName) {
        List<NewsItemDto> items = new ArrayList<>();
        items.add(news("int-daily",
                "Daily standup сегодня в 09:00 — канал " + profile.slackChannel(),
        ...
        items.add(news("int-welcome",
                playerName + ", добро пожаловать в команду — " + profile.yourRole(),
